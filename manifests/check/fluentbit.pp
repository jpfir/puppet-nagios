class nagios::check::fluentbit (
  ...
  file { '/usr/lib64/nagios/plugins/check_fluentbit_health.sh':
    ensure => $ensure,
    owner  => 'root',
    group  => 'root',
    mode   => '0755',
    source => "puppet:///modules/${module_name}/scripts/check_fluentbit_health.sh",
  }

  nagios::client::nrpe_file { 'check_fluentbit_health':
    ensure => $ensure,
    args   => $fullargs,
    command => '/usr/lib64/nagios/plugins/check_fluentbit_health.sh',
  }

  nagios::service { "check_fluentbit_health_${check_title}":
    ensure                   => $ensure,
    check_command            => 'check_nrpe!check_fluentbit_health',
    service_description      => 'Fluent Bit Health Check',
